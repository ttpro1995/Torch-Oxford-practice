...
opt.batch_size = 3
opt.train_size = 3000  -- set to 0 or 60000 to use all 60000 training data
opt.test_size = 3000      -- 0 means load all data
...
local test_losses = {}     -- [Meow] test losses for each iteration/minibatch 
...
for i = 1, iterations do
 ...
 -- eval the test loss
  ...
  -- 2. compute the loss of these outputs, measured against the true labels in batch_target
  local test_loss = criterion:forward(test_outputs, test_targets)
...
gnuplot.plot({
  ...
  '-'},
   { --[meow] plot the test loss on same figure
...
-- [meow]
cp = classPredictions 
cp = cp[{{} ,1  }] -- change size from 3000x1 to 3000 
cp = cp:byte() -- change data type to match lb
lb = test.labels  
local correct = cp:eq(lb) -- 1 for correct, 0 for incorrect
local percentage = correct:sum()/correct:size(1)
print 'acc' 
print (percentage)
